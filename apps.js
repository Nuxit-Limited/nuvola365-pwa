...
const Applications = {
  ...
  'cloud-pc': {
    ...
    openInTab: true,
    // Direct launch support: https://windows.cloud.microsoft/?cloudPcId=<id>
    directLaunchSupported: true,
    getDirectLaunchUrl: function(cloudPcId) {
      return cloudPcId 
        ? `https://windows.cloud.microsoft/?cloudPcId=${cloudPcId}`
        : this.url;
    }
  },
  ...
  'avd': {
    ...
    icon: 'fas fa-cloud',
    color: '#0078d4',
    description: 'Remote apps and desktops (HTML5 Web Client)',
    isCloudApp: true,
    url: 'https://client.wvd.microsoft.com/arm/webclient/',
    openInTab: true,
    // Direct launch support: ?workspaceId=<id>&resourceId=<id>
    directLaunchSupported: true,
    getDirectLaunchUrl: function(workspaceId, resourceId) {
      if (workspaceId && resourceId) {
        return `${this.url}?workspaceId=${workspaceId}&resourceId=${resourceId}`;
      }
      return this.url;
    }
  },
...
// ============================================
// CLOUD CONFIGURATION
// Direct Launch URLs for Windows 365 & AVD
// ============================================

/* 
 * Configure your direct launch IDs here for one-click access
 * 
 * WINDOWS 365:
 * Get your Cloud PC ID from: https://windows.cloud.microsoft/
 * Format: cloudPcConfig.cloudPcId = "your-cloud-pc-id-here"
 * 
 * AZURE VIRTUAL DESKTOP:
 * Get your IDs using Azure PowerShell:
 * - Workspace ID: Get-AzWvdWorkspace
 * - Resource ID (Desktop): Get-AzWvdDesktop
 * - Resource ID (App): Get-AzWvdApplication
 * 
 * See: https://learn.microsoft.com/en-us/windows-app/direct-launch-urls
 */

const CloudConfig = {
  windows365: {
    // Set your Cloud PC ID here for direct launch
    cloudPcId: null, // Example: "12345678-1234-1234-1234-123456789abc"
    
    // Get the launch URL
    getLaunchUrl: function() {
      const app = Applications['cloud-pc'];
      if (this.cloudPcId && app.getDirectLaunchUrl) {
        return app.getDirectLaunchUrl(this.cloudPcId);
      }
      return app.url;
    }
  },
  ...
  avd: {
    // Set your workspace and resource IDs here for direct launch
    workspaceId: null,  // Example: "12345678-1234-1234-1234-123456789abc"
    resourceId: null,    // Example: "87654321-4321-4321-4321-210987654321"
    
    // Get the launch URL
    getLaunchUrl: function() {
      const app = Applications['avd'];
      if (this.workspaceId && this.resourceId && app.getDirectLaunchUrl) {
        return app.getDirectLaunchUrl(this.workspaceId, this.resourceId);
      }
      return app.url;
    }
  },
  
  // Helper function to launch with configured IDs
  launchCloudApp: function(appId) {
    if (appId === 'cloud-pc' && this.windows365.cloudPcId) {
      window.open(this.windows365.getLaunchUrl(), '_blank');
      return true;
    } else if (appId === 'avd' && this.avd.workspaceId && this.avd.resourceId) {
      window.open(this.avd.getLaunchUrl(), '_blank');
      return true;
    }
    return false;
  }
...
// Make CloudConfig globally available
